            ...
            if (!node.empty())
            {
                ...
                if (!value.empty())
                {
                    if (value.isReal())
                        ...
                        defaultVal = cv::format("%d", (int)value);
                    else if (value.isSeq())
                    {
                        for (size_t i = 0; i < value.size(); ++i)
                        {
                            cv::FileNode v = value[(int)i];
                            if (v.isInt())
                                defaultVal += cv::format("%d ", (int)v);
                            else if (v.isReal())
                                defaultVal += cv::format("%f ", (float)v);
                            else
                                CV_Error(cv::Error::StsNotImplemented, "Unexpected value format");
                        }
                    }
                    else
                        CV_Error(cv::Error::StsNotImplemented, "Unexpected field format");
                }
            }
        }
    }
...
std::string findFile(const std::string& filename)
{
    if (filename.empty() || cv::utils::fs::exists(filename))
        return filename;
    ...
    for (int i = 0; i < 2; ++i)
    {
        if (extraPaths[i] == NULL)
            continue;
        std::string absPath = cv::utils::fs::join(extraPaths[i], cv::utils::fs::join("dnn", filename));
        if (cv::utils::fs::exists(absPath))
            return absPath;
    }
    CV_Error(cv::Error::StsObjectNotFound, "File " + filename + " not found! "
