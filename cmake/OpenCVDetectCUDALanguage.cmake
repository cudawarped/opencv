...
if(CUDA_FOUND)
    ...
    if(WITH_NVCUVID)
      ...
      if(${HAVE_NVCUVID_HEADER})
        ...
        if(CUDA_nvcuvid_LIBRARY)
          ...
          message(WARNING "Library not found - WITH_NVCUVID requires Nvidia decoding library nvcuvid")
        endif()
      else()
        message(WARNING "Header not found - WITH_NVCUVID requires Nvidia decoding library header nvcuvid.h")
      endif()
    endif()
    ...
    if(WITH_NVCUVENC)
      ...
      if(${HAVE_NVCUVENC_HEADER})
        ...
        if(CUDA_nvencodeapi_LIBRARY OR CUDA_nvidia-encode_LIBRARY)
          ...
          if(WIN32)
            message(WARNING "Library not found - WITH_NVCUVENC requires Nvidia encoding library nvencodeapi.lib")
          else()
            message(WARNING "Library not found - WITH_NVCUVENC requires Nvidia encoding shared libary or stub libnvidia-encode.so")
          endif()
        endif()
      else()
        message(WARNING "Header not found - WITH_NVCUVENC requires Nvidia encoding library header nvEncodeAPI.h.")
      endif()
    endif()
  endif()
  ...
  function(parse_cmake_cuda_architectures)
    ...
    if(CMAKE_CUDA_ARCHITECTURES STREQUAL "all" OR CMAKE_CUDA_ARCHITECTURES STREQUAL "all-major" OR CMAKE_CUDA_ARCHITECTURES STREQUAL "native")
      message(WARNING "CMAKE_CUDA_ARCHITECTURES=${CMAKE_CUDA_ARCHITECTURES}, special values all, all-major and native are note supported by OpenCV, specify only CUDA real and/or virtual architectures or use CUDA_ARCH_BIN and/or CUDA_ARCH_PTX!")
      return()
    endif()
    set(internal_ptx "")
    set(internal_bin "")
    ...
    if(internal_ptx)
      set(CUDA_ARCH_PTX ${internal_ptx} CACHE STRING "Specify 'virtual' PTX architectures to build PTX intermediate code for")
    endif()
    if(internal_bin)
      set(CUDA_ARCH_BIN ${internal_bin} CACHE STRING "Specify 'real' GPU architectures to build binaries for, BIN(PTX) format is supported")
    endif()
    set(CMAKE_CUDA_ARCHITECTURES "" PARENT)
    unset(CUDA_GENERATION CACHE)
  endfunction()
  parse_cmake_cuda_architectures()
  ...
  macro(ocv_filter_available_architecture result_list)
    set(__cache_key_check "${ARGN} : ${CUDAToolkit_NVCC_EXECUTABLE} ${OPENCV_CUDA_DETECTION_NVCC_FLAGS}")
    if(DEFINED OPENCV_CACHE_CUDA_SUPPORTED_CC AND OPENCV_CACHE_CUDA_SUPPORTED_CC_check STREQUAL __cache_key_check)
      ...
      foreach(target_arch ${CC_LIST})
        ...
        set(_cmd "${CUDAToolkit_NVCC_EXECUTABLE}" ${OPENCV_CUDA_DETECTION_NVCC_FLAGS} ${NVCC_OPTION} "${OpenCV_SOURCE_DIR}/cmake/checks/OpenCVDetectCudaArch.cu" --compile)
        execute_process(
            COMMAND ${_cmd}
  ...
  macro(ocv_detect_native_cuda_arch status output)
    set(OPENCV_CUDA_DETECT_ARCHS_COMMAND "${CUDAToolkit_NVCC_EXECUTABLE}" ${OPENCV_CUDA_DETECTION_NVCC_FLAGS} "${OpenCV_SOURCE_DIR}/cmake/checks/OpenCVDetectCudaArch.cu" "--run")
...
if(HAVE_CUDA)
  ...
  if(MSVC AND CUDA_ENABLE_DELAYLOAD)
    ...
    file(GLOB CUDA_DLLS "${CUDAToolkit_LIBRARY_ROOT}/bin/*.dll")
    foreach(d ${CUDA_DLLS})
